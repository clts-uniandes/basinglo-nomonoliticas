...
class CredentialsPostgresRepository(CredentialsRepository):
    ...
    def get_by_username(self, username: str) -> Credential:
        credential_dto = (
            db.session.query(CredentialDTO).filter_by(username=username).first()
        )
    ...
    def add(self, credential: Credential):
        old_credential = (
            db.session.query(CredentialDTO)
            .filter_by(username=credential.username)
            .first()
        )
        if old_credential is not None:
            raise Exception("Credential already exist")
        credential_dto = self.credential_factory.create_object(
